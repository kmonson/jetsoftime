...
def set_stats_a(file_pointer, character, location, lost_worlds):
    global chrono, marle, lucca, robo, frog, ayla, magus, char_array
    ayla[16] = 10
    if (location == "start" or location == "start2" or location == "cathedral") and lost_worlds != "Y":
        char_array = character[1:]
        ...
        if character == chrono:
            ...
            char_array = [140, 12, 16, 15, 4, 12, 15, 11, 5, 0xF0, 0, 0xA0, 100, 2, 0xC0, 10]
        elif character == magus:
            char_array = [150, 22, 13, 11, 24, 15, 12, 32, 5, 0xF0, 0, 0xA0, 50, 2, 0xC0, 12]
    elif location == "proto" or location == "burrow" or location == "dactyl" or lost_worlds == "Y":
        if character == chrono:
            ...
            char_array = [290, 32, 31, 33, 7, 20, 23, 16, 15, 0xBE, 0x19, 1450, 200, 3, 0xE0, 10]
        elif character == magus:
            char_array = [250, 42, 26, 21, 34, 22, 19, 38, 15, 0xBE, 0x19, 1450, 100, 3, 0xE0, 12]
    write_stats(file_pointer, character, char_array)
    ayla[16] = 13
...
def write_chars(file_pointer, char_dict, locked_chars, lost_worlds, outfile):
    ...
    if locked_chars == "Y":
        ...
        patch.patch_file("locked_chars.txt", outfile)
    i = 0
    while i < 7:
...
def randomize_char_positions(outfile, locked_chars, lost_worlds):
    f = open(outfile, "r+b")
    character_locations = {"start": "", "start2": "", "cathedral": "", "castle": "", "proto": "", "burrow": "",
                           "dactyl": ""}
